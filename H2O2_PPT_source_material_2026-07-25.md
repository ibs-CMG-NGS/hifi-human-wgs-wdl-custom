...
| 파일 | 내용 |
|---|---|
| `variant_burden_plots.pdf` | SNV+indel 유전자별 burden (p-value/효과크기 분포) |
| `sv_burden_plots.pdf` | 단일샘플 SV burden |
| `h2o2_analysis_results_joint/cohort/sv_burden_plots.pdf` | joint SV burden (다카피 아티팩트가 상위를 차지하는 모습) |
| `geneset_burden_variant_plots.pdf` / `geneset_burden_sv_plots.pdf` | GO 세트 단위 burden (기존 2세트) |
| `geneset_burden_variant_curated_plots.pdf` / `geneset_burden_sv_curated_plots.pdf` | 큐레이션 20세트 확대판 |
...
모든 그림은 `h2o2_analysis_results/cohort/` 하위에 있으며(별도 표기 제외), 한글은 `cairo_pdf`+`Noto Sans CJK KR`로 렌더링, 다중패널은 patchwork로 구성.

| 파일명 | 내용 | 대응 분석 |
|---|---|---|
| `variant_burden_plots.pdf` | SNV+indel 유전자별 burden (p/효과크기 분포) | Slide 2-4/2-5b |
| `sv_burden_plots.pdf` | 단일샘플 SV burden | Slide 2-5/2-5b |
| `../../h2o2_analysis_results_joint/cohort/sv_burden_plots.pdf` | **joint SV burden** (다카피 아티팩트 상위 차지 확인용) | Slide 2-5b |
| `geneset_burden_variant_plots.pdf` / `geneset_burden_sv_plots.pdf` | GO 세트(기존 2세트) 단위 burden | Slide 2-4/2-5b |
| `geneset_burden_variant_curated_plots.pdf` / `geneset_burden_sv_curated_plots.pdf` | GO 세트 큐레이션 20세트 확대판 | Slide 2-5b |
| `dmr_A_vs_B_plots.pdf` … `dmr_C_vs_D_plots.pdf` (6쌍) | 군쌍별 차등 메틸화 영역(DMR) 결과 | Slide 2-4 |
| `trgt_group_compare_plots.pdf` | 반복서열(STR) 군간 비교 | Slide 2-2 |
| `asm/{sample}.asm_plots.pdf` (12샘플) | 샘플별 hap1 vs hap2 ASM 결과 | Part 3 방법 5 |
| `summary_compare_plots.pdf` | 샘플단위 4개 지표(변이수/SV수/hom-peak%/VAF중앙값) 군간 boxplot | Part 3 |
| `summary_compare_plots_discordance_rank.pdf` | 웰간 불일치율 barplot + 지표별 순위 일관성 heatmap | Part 3 방법 2 |
| `hap_imbalance_plots.pdf` | Haplotype 방향성 allelic imbalance 결과 | Part 3 방법 4 |
| `hp_consistency_plots.pdf` | Read 단위 haplotype 배정 일관성 + n_het_snv 상관 산점도 | Part 3 (참고지표) |
| `heterogeneity_consensus_summary.pdf` | **5개 독립 방법 종합 heatmap (핵심 결론 그림)** | Part 3 |
...
17. (Part3) **5개 독립 검증 결과 종합 (핵심 결론 슬라이드)** — Slide 3-5, `heterogeneity_consensus_summary.pdf` 삽입
